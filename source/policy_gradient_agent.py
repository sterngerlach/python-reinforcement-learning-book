...
class PolicyGradientAgent(FNAgent):
    ...
    def scaler_path(self, model_path):
        file_name, _ = os.path.splitext(model_path)
        file_name += "_scaler.pkl"
        return file_name
    ...
    def initialize(self, experiences, optimizer):
        ...
        self.model = K.models.Sequential([
            K.layers.Dense(units=10, activation="relu", input_shape=(feature_size,)),
    ...
    def update(self, states, actions, rewards):
        ...
        self._updater([normalized_states, actions, rewards])
...
class PolicyGradientTrainer(Trainer):
    ...
    def train(self, env, episode_count=220, epsilon=0.1,
              initial_count=-1, render=False):
...
def main(play):
    ...
    if play:
        ...
        trained = trainer.train(env, episode_count=750)
        trainer.logger.plot("Rewards", trainer.reward_log, trainer.report_interval)
        trained.save(path)
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Policy gradient agent")
