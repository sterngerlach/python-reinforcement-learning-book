...
class DynaAgent(object):
    ...
    def learn(self, env, episode_count=3000, gamma=0.9,
              learning_rate=0.1, steps_in_model=-1,
              report_interval=100):
        ...
        for e in range(episode_count):
            ...
            while not done:
                ...
                gain = reward + gamma * max(self.value[next_state])
                self.value[s][a] += learning_rate * (gain - self.value[s][a])
                
                # additional learning using model
                if steps_in_model > 0:
                    model.update(s, a, reward, next_state)
