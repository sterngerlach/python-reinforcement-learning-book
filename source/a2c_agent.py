...
from framework import FNAgent, Trainer, Observer, Experience
...
class ActorCriticAgent(FNAgent):
    ...
    def make_model(self, feature_shape):
        ...
        actor_layer = K.layers.Dense(
            len(self.actions), kernel_initializer=normal)
        action_evals = actor_layer(model.output)
        actions = SampleLayer()(action_evals)
        
        critic_layer = K.layers.Dense(
    ...
    def set_updater(self, optimizer,
        ...
        updates = optimizer.get_updates(
            loss=loss, params=self.model.trainable_weights)
        
        self._updater = K.backend.function(
            inputs=[self.model.input, actions, rewards],
            outputs=[loss, policy_loss,
                     tf.reduce_mean(neg_logs), tf.reduce_mean(advantages),
                     value_loss, action_entropy],
            updates=updates)
    ...
    def categorical_entropy(self, logits):
        ...
        p0 = ea0 / z0
        # return -tf.reduce_sum(p0 * tf.log(p0), axis=-1)
...
class ActorCriticTrainer(Trainer):
    def __init__(self, buffer_size=50000, batch_size=32,
                 gamma=0.99, initial_epsilon=0.1, final_epsilon=1e-03,
                 learning_rate=1e-03, report_interval=10,
                 log_dir="", file_name=""):
        ...
        self.losses = {}
        self._max_reward = -10.0
        
    def train(self, env, episode_count=1200, initial_count=10,
              test_mode=False, render=False, observe_internal=100):
        actions = list(range(env.action_space.n))
    ...
    def step(self, episode, step_count, agent, experience):
        if self.training:
            loss, loss_policy, loss_action, loss_advantage, \
                loss_value, entropy = agent.update(*self.make_batch())
            self.losses["loss"].append(loss)
            self.losses["loss_policy"].append(loss_policy)
            self.losses["loss_action"].append(loss_action)
            self.losses["loss_advantage"].append(loss_advantage)
            self.losses["loss_value"].append(loss_value)
            self.losses["entropy"].append(entropy)
            
    def make_batch(self):
        batch = random.sample(self.d_experiences, self.batch_size)
        states = np.array([e.s for e in batch])
        actions = np.array([e.a for e in batch])
        rewards = np.array([e.r for e in batch])
    ...
    def episode_end(self, episode, step_count, agent):
        ...
        for t, r in enumerate(rewards):
            future_r = [_r * (self.gamma ** i)
                        for i, _r in enumerate(rewards[t:])]
            _r = sum(future_r)
        ...
        if self.training:
            ...
            for k in self.losses:
                ...
                self.logger.write(self.training_count, "loss/" + str(k), loss)
